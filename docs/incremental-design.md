...
## 5. 桥：审计事件 → 评审队列（2026-08-11 上线）

审计流水只有文件名+数字空间 ID，没有 nodeId。桥（app/audit_bridge.py）不做
文件名启发式主键，而把事件当**门铃**：

1. 消费未处理的知识库写事件（action 含"知识库"或 module=团队空间）
2. space_map 学习映射：事件资源名与镜像文档**唯一**同名连接 → 记住
   space_id↔workspaceId（歧义不学，月度对账仲裁；映射是静态的，学一次管终身）
3. 映射且在范围内（KG_BRIDGE_SCOPE：watched=已治理库 / mapped=任意已映射库）
   → 去抖（默认 15 分钟）触发一次 bridge 模式单库遍历 → node_id 精确
   新增/变更判定 → 入评审队列
4. 资源名唯一时回填事件 matched_node_id，审计行与节点互链

护栏：file_class 分类（fileclass.py，工程残留/图片/媒体/压缩包不入评审队列）；
KG_NOTIFY_WORKSPACES 推送白名单（未接入的库只记 skipped 不发 DM）。

成本：只有"有动静的库"才走一遍——安静的组织只花 CDC 拉取本身（~0.2%/月）。
