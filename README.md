...
- **AI 评审**：V1.1 规则引擎（7 维度 24 条，参数可配置）+ 可选大模型正文评审（`KG_MODEL_ALLOW_CONTENT_TRANSFER`），按上传人一级部门解析规则（部门覆盖 → 全局 → 内置），实例不可变、逐条留痕 `rule_config_ref` 与正文不可用原因 `content_note`。数字员工（机器人）上传不自动参与评审（详情页手动重评为明确人为意图，放行）。
